...
function ModalComponent({ show, handleClose, modalTitle, modalContent, error, modalSize, position }) {
  if (!show) {
    return null;
  }
  ...
  const modalClassName = error ? 'custom-modal error' : 'custom-modal';
  const modalSizeClass = modalSize ? `modal-${modalSize}` : ''; // Adiciona a classe de tamanho do modal se fornecida
  const positionClass = position === 'right' ? 'modal-right' : ''; // Adiciona a classe de posição do modal

  return (
    <div className={modalClassName}>
      <div className={`custom-modal ${modalSizeClass} ${positionClass}`}>
        <div className="custom-modal-content">
          <div className="custom-modal-header">
            <h5 className="modal-title">{modalTitle}</h5>
            <button type="button" className="btn-close" onClick={handleClose} aria-label="Close">X</button>
